...
function rss2post_options(){
	...
	if( isset($_POST["submit"]) ):
    	...
    	echo '<p><strong>Your settings have been updated</strong></p>';
    	echo '</div></p>';
 	endif;
	...
	echo '<div class="wrap">';
	echo '<h2>Configure Your Meetup.com options.</h2>';
	echo '<p><em>example: http://meetup.orlandophp.org/calendar/rss/The+Orlando+PHP+Meetup+Group</em></p>';
	echo '<form method="post" action='. $_SERVER['PHP SELF'] .'?page='. plugin_basename(__FILE__).'>';
	echo '<label>RSS Feed URL</label>';
	echo '<input type="text" name="feedurl" size="60" value='. $feedurl .'><br />';
	...
	while($meta_exists = mysql_fetch_array($existing_meta)){
 	...
 	foreach($rss->xpath('channel/item') as $item)
 	{
		...
		if(in_array($item->guid, $guid_r, false)){
			...
			$meta_query = "SELECT * FROM wp_postmeta WHERE meta_value = '$rss_guid' ";
			$meta_result = mysql_query($meta_query);
			$meta_row = mysql_fetch_array($meta_result);
			$update_post_id = $meta_row['post_id'];
